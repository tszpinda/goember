...
func (m *Marshaler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if "PATCH" == r.Method || "POST" == r.Method || "PUT" == r.Method {
		...
		if !strings.HasPrefix(
			...
			writeJSONError(w, tt.NewHTTPEquivError(NewMarshalerError(
		...
		//use type to accept following json, ie having User type as parameter
		// it will expect following json:
		// {"user": {"name": "John", email: "john@gm.co"}}
		mrq := make(map[string]interface{})
		mrq[strings.ToLower(m.v.Type().In(2).Kind().String())] = rq
		rqq := reflect.ValueOf(&mrq)
		fmt.Printf("in[2]: %+v\n", rqq)
		fmt.Printf("in[2]: %+v\n", m.v.Type().In(2))

		out := decoder.MethodByName("Decode").Call([]reflect.Value{rqq})

		//		out := decoder.MethodByName("Decode").Call([]reflect.Value{rq})
		if !out[0].IsNil() {
			writeJSONError(w, tt.NewHTTPEquivError(
	...
	if !out[3].IsNil() {
		...
		if _, ok := err.(tt.HTTPEquivError); ok {
			writeJSONError(w, err)
		} else {
			writeJSONError(w, tt.NewHTTPEquivError(err, code))
		}
		return
	}
	if nil != header {
	...
	if nil != rs && http.StatusNoContent != code {
		mrq := make(map[string]interface{})
		returnType := reflect.TypeOf(rs).String()
		lInd := strings.LastIndex(returnType, ".")
		if lInd > -1 {
			lInd++
			returnType = returnType[lInd:len(returnType)]
		}
		mrq[strings.ToLower(returnType)] = rs

		if err := json.NewEncoder(w).Encode(mrq); nil != err {
			log.Println(err)
		}
	}
}
